...
static void dump_results(void)
{
  ...
  struct Entry sorted_qsort[2*MAX_BUCKETS];
  struct Entry sorted_radix[2*MAX_BUCKETS];
  struct Entry sorted_slpr[2*MAX_BUCKETS];
  ...
  qsort(sorted_qsort, 2*MAX_BUCKETS, sizeof(struct Entry), cmp_entries);
  ...
  clock_gettime(CLOCK_MONOTONIC, &start);
  radix_sort(sorted_radix, 2*MAX_BUCKETS, sk.f_max);
  clock_gettime(CLOCK_MONOTONIC, &end);
  ...
  clock_gettime(CLOCK_MONOTONIC, &start);
  slpr_sort(sorted_slpr, 2*MAX_BUCKETS, sk.f_max);
  clock_gettime(CLOCK_MONOTONIC, &end);
...
int main(int argc, char **argv)
{
  ...
  int ifindex;
  int err = 0;
  const char *obj_file = "bubble_sketch_ebpf.o";
  ...
  if (!ifindex) {
    ...
    tail_call_map_fd = bpf_map__fd(skel->maps.tail_call_map);

    /* Initialize BubbleSketch */
    struct BubbleSketch sk = {};
    ...
    /* Open BPF object file for tail calls */
...
struct {
        ...
        {"xdp_insert_hot_bucket0", skel->progs.xdp_insert_hot_bucket0, 1},
        {"xdp_insert_hot_bucket1", skel->progs.xdp_insert_hot_bucket1, 2},
        {"xdp_insert_cold", skel->progs.xdp_insert_cold, 3},
    };

    for (int i = 0; i < sizeof(tail_progs) / sizeof(tail_progs[0]); i++) {
        if (!tail_progs[i].prog) {
            fprintf(stderr, "Error: tail program '%s' not found in skeleton\n", 
                    tail_progs[i].name);
            goto cleanup;
        }

        int prog_fd = bpf_program__fd(tail_progs[i].prog);
        if (prog_fd < 0) {
            fprintf(stderr, "Error: failed to get fd for program '%s'\n", 
                    tail_progs[i].name);
            goto cleanup;
        }

        /* Update prog_array with the program fd at the specified index */
        int tail_key = tail_progs[i].index;
        if (bpf_map_update_elem(tail_call_map_fd, &tail_key, &prog_fd, BPF_ANY) != 0) {
            fprintf(stderr, "Error: bpf_map_update_elem failed for index %d (%s): %s\n",
                    tail_key, tail_progs[i].name, strerror(errno));
            goto cleanup;
        }

        printf("Inserted tail program '%s' (fd=%d) at prog_array index %d\n",
               tail_progs[i].name, prog_fd, tail_key);
    }
    /* Locate main program from skeleton and attach it */
    ...
    /* Main loop with throughput reporting */
