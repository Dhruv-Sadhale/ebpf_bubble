// BubbleSketch_pcap.c - Userspace packet capture version with per-second stats
...
#define KEY_LEN 13
#define SNAPLEN 128  // We only need headers
#define PROMISC 1
#define TIMEOUT_MS 1
...
// BubbleSketch instance
static BubbleSketch sketch;
...
static void packet_handler(u_char *user, const struct pcap_pkthdr *pkthdr,
    ...
    if (parse_packet(packet, pkthdr->caplen, &tuple) == 0) {
        ...
        BubbleSketch_Insert(&sketch, key);
        processed_packets++;
        total_inserts++;
    }
}
...
int main(int argc, char **argv) {
    ...
    printf("=== BubbleSketch Userspace Packet Capture ===\n");
    printf("Interface: %s\n", interface);
    printf("Memory: %d KB\n", MEM);
    printf("Top-K: %d\n", K);
    ...
    BubbleSketch_init(&sketch, 10, K, MEM);
    ...
    // Set filter for IPv4 only (optional, improves performance)
    ...
    while (keep_running) {
        ...
        if (current.tv_sec > last_second_time.tv_sec) {
            // Print stats for the completed second
    ...
    // Print final partial second if any packets were captured
